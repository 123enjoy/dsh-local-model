...
async function pruneOptions(cfg, cache, options) {
  ...
  let pruneEnd = messages.length - keep
  // Adjust cut point so kept never starts with a tool role message.
  // A tool message must follow an assistant message with tool_calls; if the
  // summary replaces that assistant, the API rejects the orphaned tool message.
  while (pruneEnd > 0 && messages[pruneEnd]?.role === 'tool') pruneEnd--
  // Also skip assistant messages with tool_calls that would now be orphaned
  // (their tool response is still in the pruned section).
  while (pruneEnd > 0 && messages[pruneEnd - 1]?.role === 'assistant' &&
         Array.isArray(messages[pruneEnd - 1]?.tool_calls) && messages[pruneEnd - 1].tool_calls.length > 0) pruneEnd--
  const pruned = messages.slice(0, pruneEnd)
  const kept = messages.slice(pruneEnd)
