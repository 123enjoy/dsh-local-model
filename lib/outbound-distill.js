...
async function pruneOptions(cfg, cache, options) {
  ...
  const effCap = overage > 1
    ...
    : rollCap
  // 保留区（最近 keep 条）是最后的可滚区：正常惰性滚动在 requestTokens ≤ threshold
  // 时停住，根本到不了保留区；只有当「链 + 最近 keep 条 + 固定开销」本身就超过惰性
  // 阈值时，循环滚完保留区外的消息后仍超阈值——此时用大批次一次滚过保留区最旧的
  // 最近消息（缩小有效 keep），让请求深落阈值内、长时间惰性稳定。必须用大批次：
  // 小批次逐条滚会让请求在阈值附近来回震荡、每轮都滚、尾部每轮位移、云端缓存
  // 永远冷（实测：大批次命中率 ~84% vs 小批次 ~39%）。最后 1 条消息永远原样保留
  // （循环边界 messages.length - 1），当前输入永不被摘要。
  // 滞回（hysteresis）：惰性触发是 request > threshold 才滚；但一旦本轮滚过，
  // 目标就放到 base budget（而不是 threshold），留出 threshold − budget 的回落
  // 空间。否则请求每次停在阈值下沿，隔一两轮又超阈值、又滚——「两次滚动间隔
  // 短」正是零滞回导致的：滚完请求 ≈ threshold，任何增长立刻再触发。
  const budgetTokens = cfg.pruneBudgetTokens
  const keepBoundary = messages.length - keep
  const keepRollCap = Math.max(1, Math.min(cfg.pruneBatchSize ?? 12, keep))
  let boundary = chain.ids.length
  let newBlocks = 0
  let rolledThisTurn = false
  while (boundary < messages.length - 1 && newBlocks < effCap) {
    const requestTokens = chainTokens() + estimateMessagesTokens(messages.slice(boundary), factorOf(cfg)) + overhead
    if (requestTokens <= (rolledThisTurn ? budgetTokens : threshold)) break
    rolledThisTurn = true
    const inKeepZone = boundary >= keepBoundary
    const batchCap = inKeepZone ? Math.min(keepRollCap, effBatch) : effBatch
    const batchEnd = adjustBatchBoundary(messages, boundary, Math.min(boundary + batchCap, messages.length - 1))
