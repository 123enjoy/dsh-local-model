...
**保留区兜底（超载回落）**：惰性滚动假设「链 + 最近 `pruneKeepRecent` 条 + 固定开销」能落到阈值内。当最近的 N 条消息本身就快超阈值（如长工具结果、推理链很长、`pruneBudgetTokens` 相对消息偏小）时，滚完保留区外的消息请求仍超阈值——老实现到此停手，于是**每轮都滚、尾部每轮位移、云端缓存永远冷**（实测正是「链满压缩后命中率 ~1%」的元凶）。现在的处理：把保留区当作最后的可滚区，用**大批次一次性滚过**保留区最旧的最近消息（缩小有效 keep），让请求**深落阈值内、长时间惰性稳定**——实测命中率从 ~39% 回到 ~84%。代价是「保留最近 N 条完整」的保证在超载时软化：保留区按需收缩，**只有最后 1 条（当前输入）永远原样保留**；notice 卡片的「保留最近 M 条」显示的是实际保留数。收缩保留区说明预算相对会话偏小，建议调大 `pruneBudgetTokens`。

**滞回（滚动间隔）**：惰性触发是请求超 `预算 × pruneRollMargin` 才滚；但滚动目标不是「降到阈值内」而是「降到 base `pruneBudgetTokens`」，把 `预算 × (pruneRollMargin − 1)` 留成回落空间。否则请求每次停在阈值下沿，隔一两轮又超阈值又滚——「两次滚动间隔很短」就是零滞回的结果，滚完请求 ≈ 阈值，任何增长立刻再触发，尾部频繁位移、缓存频繁破（实测：无滞回命中率 83.6%，有滞回 95.3%；间隔从 ~5 轮拉到 ~7 轮，调大 `pruneRollMargin` 间隔更长）。滚动后请求落在 `预算` 附近，之后随会话增长慢慢回到阈值——这一整段字节稳定，云端缓存一直命中。
